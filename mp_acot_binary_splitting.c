...
int mp_acot_binary_splitting(mp_int * q, mp_int * a, mp_int * b, mp_int * P,
			     mp_int * Q, mp_int * R)
{
    int err;
    ...
    if ((err = mp_acot_binary_splitting(q, a, &t1, &p1, &q1, &r1)) != MP_OKAY) {
	goto _ERR;
    }
    if ((err = mp_acot_binary_splitting(q, &t1, b, &p2, &q2, &r2)) != MP_OKAY) {
	goto _ERR;
    }
