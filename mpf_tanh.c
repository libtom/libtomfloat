...
int mpf_tanh(mp_float * a, mp_float * b)
{
	...
	// use (exp(x) - exp(-x))/(exp(x) - exp(-x)) otherwise
	if ((err = mpf_init_multi(a->radix, &t1, &t2, &t3, NULL)) != MP_OKAY) {
	    return err;
	}
	...
	a->mantissa.sign = (a->mantissa.sign == MP_NEG) ? MP_ZPOS : MP_NEG;
	if ((err = mpf_sub(&t1, &t2, &t3)) != MP_OKAY) {
	    goto _SERR;
	}
	...
	if ((err = mpf_div(&t3, &t1, &t1)) != MP_OKAY) {
	    goto _SERR;
	}
    ...
    if ((err = mpf_sincos(&x, &x, 1, 1, 1)) != MP_OKAY) {
	goto _ERR;
    }
